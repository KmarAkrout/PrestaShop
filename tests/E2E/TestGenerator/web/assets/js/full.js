$(document).ready(function () {
    ...
    $('#listFolders').on('change', function () {
        var selected = '../../test/campaigns/full/' + $('#listFolders :selected').text();
        $.ajax({
            type: "POST",
            url: "/getFiles",
...
function generateSpecCmd() {
    ...
    if ("" === folder) {
        swal({
            title: "Error !",
            text: "You have to select a path, if you don't have one, you can run All compaign Full",
            icon: "error",
            button: "Okay",
        });
        return false;
    ...
    var cmd = "path=full/" + folder + file + " npm run specific-test --";
